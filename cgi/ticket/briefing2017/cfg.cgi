...
$admin_email = 'yoyaku@gekidan-curryrice.com';	# 管理者メールアドレス
					# お客様に通知されます。
$send_email = 'uchuzine@gmail.com';	# 上以外の送付先メールアドレス、客への通知なし
					# 複数設定するときはカンマ(,)でくぎる

# 予約データ格納ファイル設定
$csvmode = '0';		# 0->全データを1ファイルに。1->公演ごとにファイル生成
...
'<span style="font-size:14px;">劇団カレーライス 2017年度</span><br /><span style="font-size:24px;">「劇団説明会」</span>の<br />参加予約を行います。<br/>';

$zan= '10';		# チケットがこの枚数以下になったら残数表示
			# 表示したくない場合は0

$end= '1';		# 開演$end時間前になったら受付終了
			# ただし分切り捨て。13時30分開演の場合は、2を入力すると11時00分終了。
			# 公演前日に終了が最大設定（公演時間以上の数字入力）

$MGRFLG  = 'OFF';	# 管理フラグ（管理ボタン有／無）
