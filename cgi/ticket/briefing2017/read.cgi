...
if($ENV{'REQUEST_METHOD'} eq "POST"){
	if ($ENV{'CONTENT_LENGTH'} > 10000) { &er_("文章が長すぎます"); }
...
foreach ($page .. $page_end) {
$lines[$_] =~ s/"//g;
($date_now,$date,$mai,$name,$age,$email,$tel,$biko) = split(/,/,$lines[$_]);
$html.= "<tr><td>$name</td><td>$age歳</td><td>$gender</td><td>$mai人</td><td>$tel</td><td>$biko</td><td>$date_now</td></tr>\n";
}
$html.= "</table>\n";
$a=0;
for($i=0;$i<=$page_;$i++){
